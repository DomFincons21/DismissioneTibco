({
    ...
    SimpletoUpperCase : function(component, event, helper){
        //debugger;
    ...
    changeForm : function(component, event, helper){
        component.set("v.formFisico", event.getSource().get("v.label") == "Persona Fisica");
    ...
    toUpperCase : function(component, event, helper){
        ...
        for (var i = 0; i < fields.length; i++){
            ...
            else if(fields[i].get("v.name") == "formType")
        ...
        if(!$A.util.isEmpty(codiceFiscale))
            helper.validateCF(component, event, false); // gia c'era
        
        if(formType == true){
            if(!$A.util.isEmpty(codiceFiscaleAccount))
                helper.validateCodiceFiscaleAccount(component, event, codiceFiscaleInput);
            if(!$A.util.isEmpty(partitaIVA))
    ...
    validateCF : function(component, event, helper){
        helper.validateCF(component, event, false);   //false se non è Account
        
        var codiceFiscaleInput;  // da qui aggiunto tb
        var fields = component.find("field");
        for (var i = 0; i < fields.length; i++){
            if(fields[i].get("v.name") == "codiceFiscaleAccount")
                codiceFiscaleInput = fields[i];
        ...
        if(formType == true){
            helper.validateCodiceFiscaleAccount(component, event, codiceFiscaleInput);  // tb tutto da var codiceFiscaleInput
        }
    ...
    getAllEvtParam : function (component, event, helper) {
        ...
        helper.changefield(component, event);//tb
    },
    ...
    validateCodiceFiscaleAccount : function(component, event, helper){
        ...
        var fields = component.find("field");
        for (var i = 0; i < fields.length; i++){
            if(fields[i].get("v.name") == "nazionalità"){
                if(fields[i].get("v.label") == "Nazionalità")
                    var nazionalità = fields[i].get("v.value");
                else if(fields[i].get("v.label") == "Sede legale di rappresentanza")
                    var nazionalitàSede = fields[i].get("v.value") } // Non bisogna inserire il value senno non prende il reportvalidity
            else if(fields[i].get("v.name") == "codiceFiscaleAccount")
                var codiceFiscaleAccount = fields[i];
        ...
        // fino a qui
        if(nazionalitàSede != ''){
            helper.validateCodiceFiscaleAccount(component, event, codiceFiscaleAccount); // cambiato il null
            helper.changefield(component, event);
            if(component.get("v.account.PED_IsEnte__c")){
                helper.validatePartitaIva(component, event, false);
                helper.changefield(component, event);}
        } // poi qui
        else {
            codiceFiscaleAccount.setCustomValidity("Selezionare la sede Legale");
            codiceFiscaleAccount.reportValidity(); }
    },
    ...
    validateCFPartitaIVA : function(component, event, helper){
        ...
        if(!$A.util.isEmpty(account.CodiceFiscale__c)){
            ...
            for (var i = 0; i < fields.length; i++){
                if(fields[i].get("v.name") == "codiceFiscaleAccount")
                    var codiceFiscaleInput = fields[i];
                else if(fields[i].get("v.name") == "partitaIVA")
                    var partitaIVA = fields[i];
                    else if(fields[i].get("v.name") == "nazionalità"){
                        if(fields[i].get("v.label") == "Sede legale di rappresentanza")
                            var sedelegale = fields[i].get("v.value");}
            }
            if(sedelegale != ''){
                helper.validateCodiceFiscaleAccount(component, event, codiceFiscaleInput);}
            else {
                codiceFiscaleInput.setCustomValidity("Selezionare la sede Legale");
        ...
        if(sedelegale != ''){
            if(!$A.util.isEmpty(account.PartitaIVA__c)){
                ...
                partitaIVA.setCustomValidity("Selezionare la sede Legale");
                partitaIVA.reportValidity();}
    },
    ...
    clickRegistraFisica : function(component, event, helper){
        component.set("v.Spinner", true);
        if(!component.get("v.captchaValid")){
            var formMessageFisica = component.get("v.formMessageFisica");
            if(!$A.util.isEmpty(formMessageFisica) && formMessageFisica != 'Scaduto')
                helper.handleValueChangeFisicaCaptcha(component, event);
            else
                component.find("captchaIdFisica").validateToken();
        }
        else
            helper.savePersonaFisicaRecord(component, event);
    },

    handleValueChangeFisicaCaptcha : function(component, event, helper){;
        helper.handleValueChangeFisicaCaptcha(component, event);
    },
